 ...
 * @returns void
 */
export const checkArgs = (args: any, argv: Array<string>): void => {
  ...
  if (derivationMode) {
    ...
    if (currency) {
      ...
      const configuredCurrency: Currency | undefined = Object.entries(
        currencies
      )
        .filter(
          (c) => c[1].symbol.toUpperCase() === args.currency.toUpperCase()
      ...
      if (!configuredCurrency) {
        throw new Error(
          `Currency ${args.currency} has not been implemented yet`
        );
      }
