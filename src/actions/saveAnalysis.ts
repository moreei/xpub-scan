...
function save(meta: any, data: any, directory: string) {
  ...
  const addresses: Array<Address> = data.addresses.map((e: Address) => {
    return {
      ...
      funded: toBaseUnit(e.getStats().funded),
      spent: toBaseUnit(e.getStats().spent),
    };
  });
  ...
  if (configuration.currency.utxo_based) {
    utxos = data.addresses
      ...
      .map((e: Address) => {
        const firstTransaction = e.getTransactions()[0];
        return {
          derivationMode: e.derivationMode,
          derivation: e.getDerivation(),
          address: e.toString(),
          ...
          txid:
            balanceOnly || !firstTransaction
              ? undefined
              : firstTransaction.txid,
          height:
            balanceOnly || !firstTransaction
              ? undefined
              : firstTransaction.blockHeight,
          time:
            balanceOnly || !firstTransaction
              ? undefined
  ...
  const transactions: Array<Operation> = !balanceOnly
    ? data.transactions.map((e: Operation) => {
        return {
          ...e,
          cashAddress: toUnprefixedCashAddress(e.getAddress()),
      ...
      ? data.comparisons.map((e: Comparison) => {
          return {
            ...
            actual: e.actual
              ? {
                  ...e.actual,
                  cashAddress: toUnprefixedCashAddress(e.actual.getAddress()),
