    ...
    if (typeof tx.ins !== "undefined" && tx.ins.length > 0) {
      if (accountNumber === 1) {
        ...
        for (const txin of tx.ins) {
          if (allOwnAddresses.includes(txin.getAddress())) {
...
function processSentTransactions(address: Address, ownAddresses: OwnAddresses) {
  ...
  for (const tx of transactions) {
    ...
    outs.forEach((out) => {
      ...
      const isInternalAddress = internalAddresses.includes(out.getAddress());
      const isExternalAddress = externalAddresses.includes(out.getAddress());
...
function compareOpsByBlockThenDate(A: Operation, B: Operation) {
  // block number
  if (A.getBlockNumber() > B.getBlockNumber()) {
    return -1;
  }
...
function getSortedOperations(addresses: Array<Address>): Array<Operation> {
  ...
  ([] as Array<Address>).concat(addresses).forEach((address: Address) => {
    ...
    address.getSentOperations().forEach((op: Operation) => {
      ...
      if (!processedTxids.includes(op.getTxid())) {
        op.setAddress(address.toString());
        ...
        operations.push(op);
        processedTxids.push(op.getTxid());
      }
    });
  });
