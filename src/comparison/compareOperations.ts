...
const showOperations = (
  ...
  if (!A.date || !A.address || !A.amount || !A.operationType) {
    return;
  }
  ...
  switch (status) {
    ...
    case status.match(/^Mismatch.*/)?.input:
      ...
      if (B && B.date && B.address && B.amount) {
        actual = B.date
          .padEnd(24, " ")
  ...
  if (
    A.operationType.includes("token") ||
    (B?.operationType && B.operationType.includes("token"))
  ) {
    actual = chalk.white(actual.concat("\t[token]"));
  }

  if (
    A.operationType.includes("dapp") ||
    (B?.operationType && B?.operationType.includes("dapp"))
  ) {
    actual = chalk.white(actual.concat("\t[dapp]"));
  }

  if (
    A.operationType.includes("SCI") ||
    (B?.operationType && B?.operationType.includes("SCI"))
  ) {
  ...
  if (
    ...
    (B?.operationType && B?.operationType.includes("Swapped"))
  ) {
...
const checkImportedOperations = (
  ...
  if (partialComparison) {
    ...
    importedOperations = importedOperations.filter((op) =>
      op
        .getAddress()
        .split(",")
        .find((a) => rangeAddresses.includes(a))
    );
  }
  ...
  for (const comparingCriterion of allComparingCriteria) {
    ...
    for (const imported of importedOps) {
      ...
      const actual = actualOperations.filter(
        (op) =>
          imported.getAddress().includes(op.getAddress()) &&
          op.amount === imported.amount
      );

      if (Object.keys(actual).length === 1 && actual[0]) {
        imported.setAddress(actual[0].getAddress());
      }
    }
    ...
    for (const criterion of allComparingCriteria) {
      ...
      for (const importedOp of imported) {
        for (let i = 0; i < actualOps.length; ++i) {
          ...
          let actualOp = actualOps[i];
          let firstOp = actualOps[0];
          if (!actualOp || !firstOp) continue;

          if (
            // [ an actual operation has the same txid,
            (actualOp.txid === importedOp.txid &&
              // and:
            ...
            (importedOp.getAddress() &&
              !importedOp.getAddress().includes(actualOp.getAddress()))
          ) {
            // ... then swap it with first actual operation
            [firstOp, actualOp] = [actualOps[i], actualOp];
            break;
          }
        }
      }
    ...
    for (let i = 0; i < Math.max(importedOps.length, actualOps.length); ++i) {
      ...
      if (
        actualOp &&
        importedOp &&
        (aggregatedTxids.includes(actualOp.getTxid()) ||
        ...
        aggregatedTxids.push(actualOp.getTxid());
      ...
      if (!importedOp || !actualOp) throw new Error("importedOp is undefined");
  ...
  for (let i = comparisons.length - 1; i >= 0; i--) {
    ...
    if (comparison && comparison.status === "Skipped") {
  ...
  skippedComparisons.sort((a, b) => {
    return a.actual!.getDate() > b.actual!.getDate() ? -1 : 1;
  });
